...
interface Session {
  id: string;
  status: 'idle' | 'running';
  history: { timestamp: Date; transcription: string; result: MatchCandidate | null; autoDisplayed: boolean }[];
}
...
export default function OperatorPage() {
  ...
  const [session, setSession] = useState<Session>({ id: '', status: 'idle', history: [] });
  ...
  useEffect(() => {
    if (mounted) {
      send({ type: 'display_update', surah, ayah, displayMode });
    }
  }, [surah, ayah, displayMode, connected]);
  ...
  const changeSurah = useCallback((s: number) => {
    ...
    if (info) broadcast(s, 1, displayMode);
  }, [broadcast, displayMode]);

  const changeAyah = useCallback((a: number) => {
    setAyah(a);
    broadcast(surah, a, displayMode);
  }, [broadcast, surah, displayMode]);

  const changeMode = useCallback((m: DisplayMode) => {
    setDisplayMode(m);
    broadcast(surah, ayah, m);
  }, [broadcast, surah, ayah]);

  // Session
  const startSession = async () => {
    const newSession: Session = { id: Date.now().toString(36), status: 'running', history: [] };
  ...
  const startListening = () => {
    ...
    recognition.onresult = async (event: SpeechRecognitionEvent) => {
      ...
      if (text) {
        setTranscription(text + (interim ? ' ' + interim : ''));
        try {
          ...
          if (data.matches?.length > 0) {
            ...
            const entry = { timestamp: new Date(), transcription: text, result: data.matches[0], autoDisplayed: false };
            setSession(prev => ({ ...prev, history: [entry, ...prev.history] }));
            setStats(prev => ({ ...prev, total: prev.total + 1 }));

            if (autoMode && data.matches[0].confidence >= 0.85) {
              setSurah(data.matches[0].surah);
              setAyah(data.matches[0].ayah);
              broadcast(data.matches[0].surah, data.matches[0].ayah, displayMode);
              setStats(prev => ({ ...prev, autoDisplay: prev.autoDisplay + 1 }));
              entry.autoDisplayed = true;
            }
  ...
  const exportCSV = () => {
    ...
    session.history.forEach(e => {
      rows.push([e.timestamp.toISOString(), e.transcription, String(e.result?.surah || ''), String(e.result?.ayah || ''), String(e.result?.confidence || ''), e.autoDisplayed ? 'Ya' : 'Tidak']);
    });
    const csv = rows.map(r => r.map(c => '"' + c + '"').join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'qlvrs-session-' + session.id + '.csv'; a.click();
    ...
    <div className='flex h-screen bg-gray-50'>
      <div className='w-80 bg-gray-900 text-white flex flex-col overflow-y-auto'>
        ...
        <div className='p-4 border-b border-gray-700 space-y-1'>
          <div className='flex items-center justify-between'>
            <h1 className='text-lg font-bold text-green-400'>Operator</h1>
            <div className='flex items-center gap-2'>
              {connected && <span className='text-xs text-green-500'>{clientCount.displays} display</span>}
          ...
          <p className='text-xs text-gray-500'>Auto: {autoMode ? 'ON' : 'OFF'} &middot; Mode: {displayMode} &middot; WS: {connected ? 'OK' : '...'}</p>
          <p className='text-xs text-gray-600'>&uarr;&darr; surah &middot; &larr;&rarr; ayat &middot; 123 candidate &middot; A auto &middot; M mode</p>
        </div>
        ...
        <div className='p-4 border-b border-gray-700 space-y-3'>
          ...
          <div className='flex gap-2'>
            {session.status === 'idle' ? (
              <button onClick={startSession} className='flex-1 py-2 bg-green-700 hover:bg-green-600 rounded text-sm font-medium'>Mulai Sesi</button>
            ) : (
              <button onClick={stopSession} className='flex-1 py-2 bg-red-700 hover:bg-red-600 rounded text-sm font-medium'>Hentikan</button>
            )}
          </div>
        </div>
            ...
            <div className='space-y-1'>
              {candidates.map((r, i) => (
                <button key={i} onClick={() => { setSurah(r.surah); setAyah(r.ayah); broadcast(r.surah, r.ayah, displayMode); }}
                  ...
                  <span className='block font-medium'>[{i + 1}] {r.surah}. {r.surah_name} : {r.ayah}</span>
                  <span className='block text-right text-sm mt-0.5' dir='rtl'>{r.text_arabic}</span>
        ...
        {session.status === 'running' && session.history.length > 0 && (
          <div className='p-4 border-b border-gray-700'>
            ...
            <div className='grid grid-cols-3 gap-2 text-center'>
              <div className='bg-gray-800 rounded p-2'><div className='text-lg font-bold text-white'>{stats.total}</div><div className='text-xs text-gray-400'>Total</div></div>
              <div className='bg-gray-800 rounded p-2'><div className='text-lg font-bold text-green-400'>{stats.autoDisplay}</div><div className='text-xs text-gray-400'>Auto</div></div>
              <div className='bg-gray-800 rounded p-2'><div className='text-lg font-bold text-yellow-400'>{stats.total > 0 ? Math.round(stats.autoDisplay / stats.total * 100) : 0}%</div><div className='text-xs text-gray-400'>Rate</div></div>
            </div>
          </div>
        )}
        ...
        {session.history.length > 0 && (
          <div className='p-4 border-b border-gray-700'>
            <div className='flex items-center justify-between mb-2'>
              <label className='text-xs text-gray-400'>Riwayat:</label>
              <button onClick={exportCSV} className='text-xs text-green-500 hover:text-green-400'>Export CSV</button>
            </div>
            <div className='space-y-1 max-h-40 overflow-y-auto'>
              {session.history.slice(0, 10).map((e, i) => (
                <div key={i} className='bg-gray-800 rounded px-2 py-1 text-xs flex items-center gap-2'>
                  <span className={'w-1.5 h-1.5 rounded-full ' + (e.autoDisplayed ? 'bg-green-500' : 'bg-gray-500')} />
                  <span className='text-gray-400 w-12'>{e.timestamp.toLocaleTimeString()}</span>
                  <span className='text-white flex-1 truncate' dir='rtl'>{e.transcription}</span>
                  {e.result && <span className='text-green-400'>{e.result.surah}:{e.result.ayah}</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className='p-4 space-y-3 border-b border-gray-700'>
          <div>
            <label className='text-xs text-gray-400 mb-1 block'>Surah (&uarr;&darr;)</label>
            <SurahPicker selected={surah} onSelect={changeSurah} />
          </div>
          <div>
            <label className='text-xs text-gray-400 mb-1 block'>Ayat (&larr;&rarr;)</label>
            <div className='flex gap-2'>
              <button onClick={() => changeAyah(Math.max(1, ayah - 1))} className='px-2 py-1 bg-gray-700 rounded hover:bg-gray-600 disabled:opacity-30' disabled={ayah <= 1}>&#8592;</button>
              <select value={ayah} onChange={(e) => changeAyah(Number(e.target.value))} className='flex-1 px-2 py-1 rounded bg-gray-800 border border-gray-700 text-white text-sm'>
                {Array.from({ length: ayahCount }, (_, i) => i + 1).map((n) => (<option key={n} value={n}>Ayat {n}</option>))}
              </select>
              <button onClick={() => changeAyah(Math.min(ayahCount, ayah + 1))} className='px-2 py-1 bg-gray-700 rounded hover:bg-gray-600 disabled:opacity-30' disabled={ayah >= ayahCount}>&#8594;</button>
            </div>
          </div>
          <div>
            ...
            <div className='flex rounded overflow-hidden border border-gray-700'>
              {(['mushaf', 'presentation', 'obs'] as const).map((m) => (
                <button key={m} onClick={() => changeMode(m)} className={'flex-1 py-1 text-xs ' + (displayMode === m ? 'bg-amber-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700')}>
          ...
          <div className='flex flex-wrap gap-1 max-h-28 overflow-y-auto'>
            {pageVerses.map((v) => (
              <button key={v.id} onClick={() => { setSurah(v.surah); setAyah(v.ayah); broadcast(v.surah, v.ayah, displayMode); }}
                className={'px-2 py-0.5 text-xs rounded ' + (v.surah === surah && v.ayah === ayah ? 'bg-green-600 text-white animate-pulse' : 'bg-gray-800 text-gray-300 hover:bg-gray-700')}>
                {v.surah}:{v.ayah}
              </button>
            ))}
          </div>
      ...
      {mounted && (
        <div className='flex-1 bg-black flex flex-col'>
          <div className='flex items-center gap-2 px-4 py-1.5 bg-gray-800 text-xs text-gray-400'>
            <span>Preview:</span>
            <a href={displayUrl} target='_blank' className='text-green-400 hover:underline truncate' rel='noreferrer'>{displayUrl}</a>
            <span className='flex-1' />
            {connected && <span className='text-green-500 flex items-center gap-1'><span className='w-1.5 h-1.5 rounded-full bg-green-500' /> WS</span>}
            {session.status === 'running' && <span className='text-green-500 flex items-center gap-1'><span className='w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse' /> LIVE</span>}
          </div>
